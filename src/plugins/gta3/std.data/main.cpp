...
// -------------------------------------------------------------------------
// [GLOBAL VARIABLES AND FUNCTIONS FOR CONFLICT DETECTION]
// -------------------------------------------------------------------------
...
// Parses a line and returns the ID (or -1 if invalid)
static int ParseIDFromLine(const std::string& line) {
    if (line.empty() || line[0] == '#' || line[0] == ';') return -1;
    // Ignore if the line does not start with a digit
    if (!isdigit(static_cast<unsigned char>(line[0]))) return -1;

    std::stringstream ss(line);
    int id;
    // Return ID only if an integer was successfully read
    if (ss >> id) return id;
    return -1;
}

// Helper function to check section headers (case-insensitive)
static bool IsSectionHeader(const std::string& line, const char* sectionName) {
    if (line.length() < 3) return false;

    // Compare the first word only
...
// Loads original game IDs from IDE files
...
// Extracts mod name from a full file path
static std::string ExtractModNameFromPath(const std::string& fullpath) {
    ...
    if (start < fullpath.length() && (fullpath[start] == '\\' || fullpath[start] == '/')) {
        start++;
    }
...
static bool CheckForConflict(int id, const std::string& currentModName, bool isIdeFile) {
    ...
    if (it == g_modIdMap.end()) {
        ...
        // Conflict or duplicate detected
        ...
        if (isExternalConflict) {
            sprintf_s(
                buffer,
                "Conflict detected: Model ID %d is defined in mod \"%s\" and mod \"%s\".",
            ...
            sprintf_s(
                buffer,
                "Duplicate detected: Model ID %d is defined twice in mod \"%s\".",
                id, currentModName.c_str()
            );
        }

        // Always log to modloader.log
        plugin_ptr->Log("%s", buffer);

        // MessageBox decision:
        // - IDE files: never show MessageBox (game can override safely)
        // - TXT files: show MessageBox, as conflicts are more risky

        if (!isIdeFile) {
            std::string msgBoxContent =
                std::string(buffer) +
                "\nGame will continue loading, but instability may occur.";
            MessageBoxA(
                NULL,
                msgBoxContent.c_str(),
                "ModLoader Conflict Detected",
